require 'cocoapods-hmap-generator/utility/xcconfig_helper'

module Pod
  class PodTarget
    def reset_target_search_path_with_hmap_path(hmap_path)
       if build_settings.instance_of?(Hash)
          build_settings.each do |config_name, setting|
            config_file = setting.xcconfig
            config_file.reset_header_search_path_to_hmap(hmap_path)
